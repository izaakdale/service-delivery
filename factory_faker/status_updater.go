...
func UpdateStatuses(in *delivery.DeliveryRequest) {
	...
	dao.UpdateStatus(&dao.DeliveryRecord{
		...
		DeliveryStatus: delivery.ORDER_STATUS_name[int32(delivery.ORDER_STATUS_OUT_FOR_DELIVERY)],
		Address:        in.Address,
	})

	interval2 := rand.Intn(20)
	time.Sleep(time.Second * time.Duration(interval2))

	logger.Info("Updating order status to delivered")
	dao.UpdateStatus(&dao.DeliveryRecord{
		OrderId:        "ORDER#" + in.OrderId,
		DeliveryStatus: delivery.ORDER_STATUS_name[int32(delivery.ORDER_STATUS_DELIVERED)],
		Address:        in.Address,
	})
}
